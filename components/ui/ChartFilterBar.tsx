  ...
  return (
    <div className="relative" ref={modelDropdownRef}>
      ...
      <button
        ...
        className={`flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 sm:rounded-full sm:border transition-all cursor-pointer ${
          modelSearchOpen
            ? 'sm:border-blue-500 sm:bg-blue-50 text-blue-600'
            : 'sm:border-gray-300 sm:bg-white sm:hover:bg-gray-50 text-gray-500 hover:text-gray-700'
        }`}
        title={`${effectiveSelectedCount} model${effectiveSelectedCount !== 1 ? 's' : ''} selected`}
      >
        <SlidersHorizontal className="w-3.5 h-3.5" />
      </button>

      {modelSearchOpen && (
        <div className="absolute top-full right-0 mt-2 w-[85vw] sm:w-[600px] max-w-[600px] bg-white border border-gray-300 rounded-md shadow-lg z-50 max-h-60 overflow-hidden flex flex-col">
          {/* Search Input */}
